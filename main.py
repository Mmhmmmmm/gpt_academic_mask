    ...
    with gr.Blocks(title="ChatGPT 学术优化", theme=set_theme, analytics_enabled=False, css=advanced_css) as demo:
        ...
        with gr_L1():
            ...
            with gr_L2(scale=1):
                ...
                with gr.Accordion("角色功能区", open=True) as area_prompt_fn:
                    dropdown_fn_list = [k for k in prompt_fns.keys()]
                    with gr.Row():
                        dropdown_pt= gr.Dropdown(dropdown_fn_list, value=r"打开角色列表", label="").style(container=False)
                    with gr.Row():
        ...
        def fn_area_visibility(a):
            ...
            ret.update({area_prompt_fn: gr.update(visible=("角色功能区" in a))})
            ret.update({area_mask_fn: gr.update(visible=("面具功能区" in a))})
        ...
        # 函数插件-下拉菜单与随变按钮的互动
        def on_prompt_dropdown_changed(k):
            variant = "secondary"
            ret = {switchy_bt_pt: gr.update(value=k, variant=variant)}
            return ret
